...
- 7/21 覆核議程：
  - 四源審判：techurls / iris(Info Flow) / 36Kr / xAI 查詢詞
    ...
    - 2026-07-19 診斷補充（V2EX 域名級排除前的快照量測）：iris 當時
      占 items_ai 總量 61.8%（110/178），且 **全數**卡在 has_ai
      地板值 0.65、僅 9 條有徽章；其中 v2ex.com 網域佔 iris 的
      44.5%（49/110＝items_ai 總量 27.5%）。V2EX 已於同日以域名級
      排除處理（見下方待辦「已結案」），iris 排除 V2EX 後裸露規模
      61 條、同樣全數壓線地板值——代表 iris 的噪音問題不只 V2EX
      一項，此為審判時的量化基準
