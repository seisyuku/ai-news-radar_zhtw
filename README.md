...
全站為純靜態網頁，由排程自動化驅動：每 30 分鐘抓取所有來源、過濾 AI 相關性、識別商業事件、統一轉換為臺灣繁體中文後發布。重點卡片在來源有提供摘要時，會以 Groq 上的 `qwen/qwen3.6-27b` 產生短篇 AI 新聞摘要；無憑證、額度不足或服務失效時會沿用快取或省略該區塊，不影響新聞更新。無伺服器、無資料庫、無追蹤，你看到的就是全部。
